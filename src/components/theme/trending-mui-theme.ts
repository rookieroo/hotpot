...
export const teal = {
  ...
  main: '#009688',
  500: '##009688',
  600: '#00897b', // vs blueDark 900: WCAG 4.6 AAA (large), APCA 36 Not for reading text
  700: '#00796b',
  800: '#00695c',
  900: '#004d40',
};
...
export const getDesignTokens = (mode: 'light' | 'dark', theme) => {
  ...
  const main = {
    50: hslaWrapper(theme.activeColor.light, 0.05),
    100: hslaWrapper(theme.activeColor.light, 0.1),
    200: hslaWrapper(theme.activeColor.light, 0.2),
    300: hslaWrapper(theme.activeColor.light, 0.3),
    400: hslaWrapper(theme.activeColor.light, 0.4),
    500: hslaWrapper(theme.activeColor.light, 0.5),
    600: hslaWrapper(theme.activeColor.light, 0.6),
    700: hslaWrapper(theme.activeColor.light, 0.7),
    800: hslaWrapper(theme.activeColor.light, 0.8),
    900: hslaWrapper(theme.activeColor.light, 0.9),
  }
  return ({
    palette: {
      primary: {
      ...
      secondary: {
        main: hslaWrapper(cssVars.secondary)
      },
      divider: hslaWrapper(cssVars.border),
      primaryDark: activeColor,
      mode,
      background: {
        default: hslaWrapper(cssVars.background),
        paper: hslaWrapper(cssVars.background, 0.8)
      },
      common: {
        black: '#0B0D0E',
      },
      text: {
        primary: hslaWrapper(cssVars.foreground),
        secondary: hslaWrapper(cssVars.foreground, 0.7),
        tertiary: hslaWrapper(cssVars.foreground, 0.6)
      },
      grey: {
        ...grey,
        ...(mode === 'light' && {
      ...
      gradients: {
        ...
        linearSubtle:
          mode === 'light'
            ? `linear-gradient(to top right, ${alpha(teal[50], 0.3)} 40%, ${alpha(
            grey[50],
            0.2,
            )} 100%)`
            : `linear-gradient(to top right, ${alpha(teal[900], 0.1)} 40%, ${alpha(
            blueDark[800],
            0.2,
            )} 100%)`,
      },
    ...
    typography: {
      ...
      h5: {
        ...
        lineHeight: 36 / 24,
        letterSpacing: 0.1,
        color: mode === 'dark' ? teal[300] : teal.main,
      },
      h6: {
...
// merge shadcn and mui theme
export const mergeThemes = (mode: 'light'|'dark', theme: string) => {
